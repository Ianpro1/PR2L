...
def make_env(ENV_NAME, inconn=None):
        ...
        if inconn is not None: # or if inconn is not None
            env = atari_wrappers.functionalObservationWrapper(env, lowsendimg(inconn, frame_skip=2)) 
...
def play_func(parameters, net, exp_queue, device, inconn=None):
        ...
        env = [env1, env2, env3]
        print(net)
        epsilon_start = parameters["EPSILON"]
        epsilon = epsilon_start

        selector = agents.EpsilonGreedySelector(epsilon_start)
        ...
        for exp in exp_source:
            ...
            for rewards, steps in exp_source.pop_rewards_steps():
                idz +=1
                epsilon = max(epsilon_start - idz/500, 0.02)
...
def calc_loss(states, actions, rewards, last_states, not_dones, tgt_net, net):
    ...
    with torch.no_grad(): #try to use numpy instead
        ...
        #bellman step
        q_v_refs = rewards + tgt_q_v * gamma # line is wrong gamma should be = gamme**N_STEPS (somehow works)
...
if __name__ == '__main__':
    ...
    p1 = tmp.Process(target=Rendering.init_display, args=(outconn, 320, 420, (210, 160))) #args=(outconn, 320, 420, (210, 160))
    p1.start()
    ...
    time.sleep(1)
    net.apply(models.network_reset)
    net.load_state_dict(torch.load("model_saves/saved/modelsave4_(13-31).pt"))
    tgt_net.sync()
